...
def convert_image_captions(html: str, theme: dict | None = None) -> str:
    """将图片后紧跟的斜体段落转为图说样式"""
    caption_config = (theme or {}).get("styles", {}).get("img_caption")
    caption_style = (
        build_style_string(caption_config)
        if caption_config
        else "text-align:center;font-size:13px;color:#999999;margin-top:-8px;margin-bottom:16px;font-style:normal"
    )
    wrapper_config = (theme or {}).get("styles", {}).get("img_wrapper", {})
    wrapper_style = build_style_string(wrapper_config)

    def replace_inline_caption(match):
        return (
            f'<section data-role="img-wrapper" style="{wrapper_style}">'
            f'{match.group(1)}<p style="{caption_style}">{match.group(2)}</p>'
            f'</section>'
        )

    html = re.sub(
        r'<p[^>]*>\s*(<img[^>]*>)\s*<br\s*/?>\s*<em[^>]*>(.*?)</em>\s*</p>',
        replace_inline_caption,
        html,
        flags=re.DOTALL,
    )
...
def _render_single_theme(tid, theme_data, gallery_html, gallery_footnote):
    ...
    rendered = convert_image_captions(rendered, theme_data)
...
def format_for_output(content: str, input_path: Path, theme: dict,
    ...
    html = convert_image_captions(html, theme)
    if footnote_html:
        footnote_html = convert_image_captions(footnote_html, theme)
