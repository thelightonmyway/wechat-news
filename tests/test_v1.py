...
class V1Tests(unittest.TestCase):
    ...
    def test_paper_prompt_uses_short_natural_style_and_verbatim_quote_rules(self):
        ...
        self.assertIn("silently build a section-to-claim map", paper_prompt)
        self.assertIn("assign each quantitative result", paper_prompt)
        self.assertIn("Preserve core quantitative evidence", paper_prompt)
        self.assertIn("Do not preview or import results", paper_prompt)
        ...
        self.assertIn("表达编辑规则", paper_prompt)
        self.assertIn("按已知信息到新信息推进句子", paper_prompt)
        self.assertIn("不把相关、伴随或一致性改写成因果", paper_prompt)
